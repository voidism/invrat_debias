            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and (global_step % args.logging_steps == 0 or step == (len(epoch_iterator)-1)):
                    ...
                    if (
                        ...
                        results = evaluate(args, model, tokenizer, prefix='step-'+str(global_step))
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        for eval_set in ['dev', 'test']:
            ...
            if args.eval_data_dir != None:
                output_eval_file = os.path.join(eval_output_dir, args.eval_data_dir.split('/')[-1][:-4]+eval_set+"_eval_results.txt")
            else:
                output_eval_file = os.path.join(eval_output_dir, eval_set+"_eval_results.txt")

            with open(output_eval_file+'-preds-%s.txt'%prefix, "w") as writer:
            ...
            with open(output_eval_file, "w") as writer:
                logger.info("***** Eval results {} {} *****".format(eval_set, prefix))
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, eval_set='dev'):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if args.eval_data_dir != None:
            ...
            examples = (
                (processor.get_dev_examples(args.data_dir) if eval_set == 'dev' else processor.get_test_examples(args.data_dir)) if evaluate else processor.get_train_examples(args.data_dir)
            )
