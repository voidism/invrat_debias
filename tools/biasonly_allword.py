...
word_types = "offensive-minority-reference offensive-not-minority harmless-minority".split(' ')
word_type = 'all' #sys.argv[5]

df = pd.read_csv(word_list)
df = df[(df.categorization==word_types[0])|(df.categorization==word_types[1])|(df.categorization==word_types[2])]
...
proba = np.log(proba)
save_file = training_path[:-4]+'_%s.pkl'%word_type
